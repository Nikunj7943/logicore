...
function tms_import_log_inject_style() {
	if (document.getElementById("tms-import-log-style")) return;
	const style = document.createElement("style");
	style.id = "tms-import-log-style";
	style.textContent = `
...
function tms_import_log_hide_native_fields(frm) {
	[
		"template",
		"template_name",
		"company",
...
function tms_import_log_render_overview(frm, summary) {
	...
	wrapper.html(`
		<div class="tms-til-hero">
			<div class="tms-til-hero-top">
				<div>
					<div class="tms-til-title">${frappe.utils.escape_html(frm.doc.template_name || __("Import Log"))}</div>
					<div class="tms-til-sub">${__("A clean summary of the imported file, mapped template columns, and row-wise outcomes for this import run.")}</div>
				</div>
...
function tms_import_log_render_summary(frm, preview, summary, rows) {
...
function tms_import_log_render_columns(frm, preview) {
...
function tms_import_log_render_rows(frm, rows, targetDoctype) {
	...
	const doctype = targetDoctype || "Trip";
	wrapper.html(`
		<div class="tms-til-panel">
					...
					<table class="tms-til-table">
						...
						<tbody>
							${rows.map(row => {
								...
								// Support both old key (trip_name) and new key (record_name) for backward compat
								const recordName = row.record_name || row.trip_name || "";
								const recordCell = recordName
									? `<a class="tms-til-trip-link" href="${frappe.utils.get_form_link(doctype, recordName)}" target="_blank">${frappe.utils.escape_html(recordName)}</a>`
									: "—";
								return `
									<tr>
...
frappe.ui.form.on("Import Log", {
	async refresh(frm) {
		try {
			tms_import_log_inject_style();
			tms_import_log_hide_native_fields(frm);
			...
			// Fetch target_doctype from the linked template for dynamic record links
			let targetDoctype = "Trip";
			if (frm.doc.template) {
				try {
					const r = await frappe.db.get_value("Import Template", frm.doc.template, "target_doctype");
					if (r && r.message && r.message.target_doctype) {
						targetDoctype = r.message.target_doctype;
					}
				} catch (e) {
					// fallback to Trip
				}
			...
			tms_import_log_render_overview(frm, summary);
			tms_import_log_render_summary(frm, preview, summary, rows);
			tms_import_log_render_columns(frm, preview);
			tms_import_log_render_rows(frm, rows, targetDoctype);
		} catch (e) {
			console.error("[TMS] Import Log render failed:", e);
			const wrapper = frm.get_field("overview_html").$wrapper;
			wrapper.html(`
				<div style="padding:14px;border:1px solid #fecaca;background:#fef2f2;color:#991b1b;border-radius:12px;font-size:13px;">
					<strong>${__("Import Log UI could not render.")}</strong><br>
