...
const _TT_SKIP_FIELDTYPES = new Set([
...
// Trip Export Columns table — Trip Type presets must offer the same columns
// as that doctype's own Export dialog.
function _tt_target_doctype(frm) {
	return frm.doc.applies_to === "Amazon" ? "Amazon Trip" : "Trip";
}

function _tt_build_cols_from_meta(doctype) {
	const meta = frappe.get_meta(doctype);
	if (!meta || !meta.fields) return [];
	return meta.fields.filter(f =>
		!_TT_SKIP_FIELDS.has(f.fieldname) &&
		!_TT_SKIP_FIELDTYPES.has(f.fieldtype) &&
		f.label
...
function _tt_update_section_label(frm) {
...
function _tt_columns_mismatch(frm, targetDoctype) {
...
function _tt_sync_columns(frm, { rebuild } = {}) {
	const targetDoctype = _tt_target_doctype(frm);

	const tryPopulate = () => {
		const cols = _tt_build_cols_from_meta(targetDoctype);
...
frappe.ui.form.on("Trip Type", {
	refresh(frm) {
		_tt_update_section_label(frm);
		if (!frm.doc.trip_export_columns || !frm.doc.trip_export_columns.length) {
			_tt_sync_columns(frm);
		} else {
			const targetDoctype = _tt_target_doctype(frm);
			const tryMismatchCheck = () => {
				if (_tt_columns_mismatch(frm, targetDoctype)) {
					_tt_sync_columns(frm, { rebuild: true });
				}
			};
			if (frappe.get_meta(targetDoctype)) {
				tryMismatchCheck();
