...
ROW_DOCTYPE = "Trip POD Import File V2"
DEFAULT_BATCH_SIZE = 10
GHOSTSCRIPT_BINARY = "/usr/bin/gs"
PDF_COMPRESSION_PROFILES = ("/ebook", "/screen")
PDF_TARGET_COMPRESSION_RATIO = 0.5
POD_MAX_FILE_SIZE = 5 * 1024 * 1024  # 5 MB — mirrors POD_MAX_BYTES in trip.js


class TripPODImportV2(Document):
...
def stage_local_folder_trip_pods(
	...
	batch_limit = max(cint(_doc_get(doc, "batch", DEFAULT_BATCH_SIZE)), 1)
	...
	for item in requested_rows:
		...
		try:
			...
			file_content = _compress_pdf_bytes(uploaded_file.stream.read())
			file_doc = save_file(
...
def import_selected_trip_pods(
	...
	for item in requested_rows:
		...
		try:
			...
			if len(file_content) > POD_MAX_FILE_SIZE:
				raise frappe.ValidationError(
					_("File '{0}' is {1:.1f} MB — maximum allowed is 5 MB.").format(
						row.file_name, len(file_content) / (1024 * 1024)
					)
				)
			...
			frappe.db.set_value(
				...
				row.matched_trip,
				{
					...
					**_get_import_pod_updates(row.matched_trip),
				},
...
def _scan_server_source_job(docname: str, trip_type: str) -> None:
	try:
		...
		doc = frappe.get_doc("Trip POD Import V2", docname)
		...
		frappe.db.rollback()
		frappe.log_error(frappe.get_traceback(), _("Trip POD Import V2 server folder scan failed"))
		_doc_db_set(
			"Trip POD Import V2",
			docname,
...
def _import_trip_pods_job(docname: str) -> None:
	try:
		...
		frappe.db.rollback()
		frappe.log_error(frappe.get_traceback(), _("Trip POD Import V2 import job failed"))
		_doc_db_set(
			"Trip POD Import V2",
			docname,
...
def _run_import(docname: str, background: bool) -> frappe._dict:
	...
	for row in doc.pod_files:
		...
		try:
			...
			if len(content) > POD_MAX_FILE_SIZE:
				raise frappe.ValidationError(
					_("File '{0}' is {1:.1f} MB — maximum allowed is 5 MB.").format(
						file_name, len(content) / (1024 * 1024)
					)
				)
			_delete_trip_pod_attachments(row.matched_trip)
			save_file(
...
def _get_import_doc(docname: str, permtype: str = "read") -> TripPODImportV2:
	if not docname:
...
def _save_and_build_preview(doc: TripPODImportV2, trip_type: str) -> frappe._dict:
	if _doc_has_field(doc, "trip_type"):
...
def _rebuild_import_rows_from_attachments(doc: TripPODImportV2) -> None:
	existing_rows = {
...
def _get_attached_import_files(doc: TripPODImportV2) -> list[frappe._dict]:
...
def _get_attached_file_doc(doc: TripPODImportV2, file_url: str):
...
def _get_attached_file_docs_by_url(doc: TripPODImportV2) -> dict[str, Document]:
...
def _build_preview(doc: TripPODImportV2, trip_type: str) -> frappe._dict:
	...
	for entry in entries:
		...
		if not reference_no:
			...
			row.status_message = _(
				"Could not parse the reference value from the file name. Use a name like 16-03-2026-12-42-31-602781.pdf."
			)
			summary.invalid_files += 1
			rows.append(row)
			continue
...
def _get_import_pod_updates(trip_name: str) -> dict[str, str]:
	current_pod_status = cstr(frappe.db.get_value("Trip", trip_name, "pod_status")).strip()

	if not current_pod_status or current_pod_status == TRIP_POD_STATUS_NOT_UPLOADED:
		return {
...
def _get_effective_trip_type(doc: TripPODImportV2, trip_type: str | None) -> str:
...
def _append_rows(doc: TripPODImportV2, rows: list[frappe._dict]) -> None:
	for row in rows:
...
def _extract_reference_no(file_name: str) -> str | None:
	base_name = os.path.splitext(os.path.basename(cstr(file_name).strip()))[0]
...
def _compress_pdf_bytes(pdf_bytes: bytes) -> bytes:
	...
	target_size = max(int(len(pdf_bytes) * PDF_TARGET_COMPRESSION_RATIO), 1)

	for profile in PDF_COMPRESSION_PROFILES:
		try:
			compressed_bytes = _run_ghostscript_compression(pdf_bytes, profile)
		except Exception:
			continue

		if not compressed_bytes or len(compressed_bytes) >= len(best_bytes):
			continue

		if not _is_readable_pdf(compressed_bytes):
			continue
...
def _run_ghostscript_compression(pdf_bytes: bytes, profile: str) -> bytes:
	with TemporaryDirectory(prefix="trip_pod_import_v2_") as temp_dir:
		...
		if result.returncode != 0:
			raise RuntimeError((result.stderr or result.stdout or "").strip() or "Ghostscript compression failed")
		if not output_path.exists():
			raise RuntimeError("Ghostscript did not produce a compressed PDF.")
		return output_path.read_bytes()


def _is_readable_pdf(pdf_bytes: bytes) -> bool:
	if not pdf_bytes:
		return False
...
def _is_server_source(doc: TripPODImportV2) -> bool:
	return cstr(_doc_get(doc, "source_mode", SOURCE_MODE_BROWSER)).strip() == SOURCE_MODE_SERVER


def _is_browser_source(doc: TripPODImportV2) -> bool:
...
def _apply_summary_to_doc(doc: TripPODImportV2, summary: frappe._dict) -> None:
...
def _build_client_payload(doc: TripPODImportV2, include_doc: bool = False) -> frappe._dict:
...
def _build_modified_response(doc: TripPODImportV2) -> frappe._dict:
...
def _add_doc_snapshot(payload: frappe._dict, doc: TripPODImportV2, include_rows: bool) -> None:
...
def _has_running_job(doc: TripPODImportV2) -> bool:
...
def _get_source_file_payload(
	doc: TripPODImportV2, row, source_files: dict[str, Document]
) -> tuple[str, bytes, int]:
	source_path = cstr(getattr(row, "source_path", "")).strip()
	if source_path:
...
def _refresh_result_report(doc: TripPODImportV2) -> str:
	# V2 no longer keeps per-import CSV reports to avoid extra storage usage.
	_clear_result_report(doc)
	return ""


def _clear_result_report(doc: TripPODImportV2) -> None:
...
def _delete_report_file(doc: TripPODImportV2, file_url: str) -> None:
	file_name = frappe.db.get_value(
		"File",
		{
...
def _delete_import_attachment(doc: TripPODImportV2, file_url: str) -> None:
	if not file_url:
		return
