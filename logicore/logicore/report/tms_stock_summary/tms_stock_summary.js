frappe.query_reports["TMS Stock Summary"] = {
	...
	formatter: function (value, row, column, data, default_formatter) {
		...
		if (column.fieldname === "actual_qty" && data) {
			const color = flt(data.actual_qty) <= 0 ? "var(--red-500)" : "var(--green-600)";
			value = `<span style="color:${color};font-weight:600;">${value}</span>`;
		} else if (column.fieldname === "received_qty" && data && flt(data.received_qty) > 0) {
			value = `<span style="color:var(--green-600);font-weight:600;">${value}</span>`;
		} else if (column.fieldname === "used_qty" && data && flt(data.used_qty) > 0) {
		...
		const downloadBtn = report.page.add_inner_button(
			...
			function () {
				const dialog = new frappe.ui.Dialog({
					title: __("Download TMS Stock Summary"),
					fields: [
						{
							fieldname: "format",
					...
					primary_action: (values) => {
						...
						if (values.format === "PDF") {
							_tms_stock_summary_download_pdf(report, values.orientation);
						} else {
							_tms_stock_summary_download_data(report, values.format);
						}
					},
				});
				dialog.show();
...
function _tms_stock_summary_download_data(report, fileFormat) {
...
function _tms_stock_summary_download_pdf(report, orientation) {
	...
	const cellColor = (fieldname, row) => {
		if (fieldname === "actual_qty") return flt(row.actual_qty) <= 0 ? "#ef4444" : "#16a34a";
		if (fieldname === "received_qty") return flt(row.received_qty) > 0 ? "#16a34a" : null;
		if (fieldname === "used_qty") return flt(row.used_qty) > 0 ? "#ef4444" : null;
		return null;
	};
...
<head>
<meta charset="UTF-8">
<title>TMS Stock Summary — ${companyName}</title>
<style>
...
<body>
  ...
  <div class="report-footer">
	...
	frappe.render_pdf(html, {
		orientation,
		report_name: `TMS_Stock_Summary_${companyName || "LogiCore"}.pdf`.replace(/\s+/g, "_"),
	});
}
