...
def sync_email_to_primary_contact(doc, method=None):
	...
	# supplier_primary_contact.email_id) -- a Property Setter (see install.py)
	# turned it into a normal editable Data field so users can type an email
	# directly on Supplier without opening the Contact. That only fixes the
	...
	if not doc.supplier_primary_contact or not doc.email_id:
		return
...
def sync_mobile_to_primary_contact(doc, method=None):
	...
	# supplier_primary_contact.mobile_no via the Contact's phone_nos child
	# table (is_primary_mobile_no flag), not email_ids.
	if not doc.supplier_primary_contact or not doc.mobile_no:
		return
